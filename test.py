...
if __name__ == '__main__':
    ...
    tophats_se = bank_of_structuring_elements(
        side_enclosing_square_in_px=9, num_orientations=8)
    inpainting_se = np.ones((5, 5), dtype='float32')

    for name in ['ISIC_0000043', 'ISIC_0000042', 'ISIC_0000074']:
        image = io.imread(f'images/{name}.jpg')
        image_hairless, steps = remove_and_inpaint(
            image, tophats_se=tophats_se, inpainting_se=inpainting_se)
